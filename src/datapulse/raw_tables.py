from pathlib import Path

from sqlalchemy import Engine, text

MIGRATIONS_DIR = Path(__file__).resolve().parents[2] / "sql" / "migrations"


def apply_raw_customers_table(engine: Engine) -> None:
    """Create the raw customers table if it does not already exist."""
    migration_path = MIGRATIONS_DIR / "003_create_raw_customers_table.sql"
    migration_sql = migration_path.read_text(encoding="utf-8")

    with engine.begin() as connection:
        connection.execute(text(migration_sql))


def apply_raw_products_table(engine: Engine) -> None:
    """Create the raw products table if it does not already exist."""
    migration_path = MIGRATIONS_DIR / "004_create_raw_products_table.sql"
    migration_sql = migration_path.read_text(encoding="utf-8")

    with engine.begin() as connection:
        connection.execute(text(migration_sql))


def apply_raw_orders_table(engine: Engine) -> None:
    """Create the raw orders table if it does not already exist."""
    migration_path = MIGRATIONS_DIR / "005_create_raw_orders_table.sql"
    migration_sql = migration_path.read_text(encoding="utf-8")

    with engine.begin() as connection:
        connection.execute(text(migration_sql))


def apply_raw_payments_table(engine: Engine) -> None:
    """Create the raw payments table if it does not already exist."""
    migration_path = MIGRATIONS_DIR / "006_create_raw_payments_table.sql"
    migration_sql = migration_path.read_text(encoding="utf-8")

    with engine.begin() as connection:
        connection.execute(text(migration_sql))


def apply_raw_subscriptions_table(engine: Engine) -> None:
    """Create the raw subscriptions table if it does not already exist."""
    migration_path = MIGRATIONS_DIR / "007_create_raw_subscriptions_table.sql"
    migration_sql = migration_path.read_text(encoding="utf-8")

    with engine.begin() as connection:
        connection.execute(text(migration_sql))


def apply_raw_support_tickets_table(engine: Engine) -> None:
    """Create the raw support tickets table if it does not already exist."""
    migration_path = MIGRATIONS_DIR / "008_create_raw_support_tickets_table.sql"
    migration_sql = migration_path.read_text(encoding="utf-8")

    with engine.begin() as connection:
        connection.execute(text(migration_sql))
